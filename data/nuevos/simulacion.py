...
def simulacion(path, instance_name):
    ...
    for route in solutions["routes"]:
        ...
        if abs(duration - (t_i - t0)) <= epsilon: 
            print("por ahora funciona. ", instance_name," ruta numero: ", idx_route)
        else: 
            duracion_nuestra= (t_i - t0)
            print("no funcionó. ", instance_name," ruta numero: ", idx_route, "duración ruta nuestra: ", duracion_nuestra, "duración posta: ", duration)
        idx_route+=1
